...
@test "M1: Non-overlapping edits merge cleanly" {
    ...
    run_refsync_with_docs "$MACHINE_DEV1" "docs" push --docs

    # Laptop pulls - should have both files
    cd_to_machine "$MACHINE_LAPTOP"
    run_refsync_with_docs "$MACHINE_LAPTOP" "docs" pull --docs
...
@test "M3: Same file with shared history - three-way merge with conflicts" {
    ...
    cd_to_machine "$MACHINE_DEV1"
    run run_refsync_with_docs "$MACHINE_DEV1" "docs" pull --docs
...
@test "M5: Shared history enables three-way merge" {
    ...
    cd_to_machine "$MACHINE_LAPTOP"
    run run_refsync_with_docs "$MACHINE_LAPTOP" "docs" pull --docs
...
@test "M6: Three machines diverged - sequential merge" {
    ...
    run_refsync_with_docs "$MACHINE_DEV2" "docs" pull --docs
    ...
    run_refsync_with_docs "$MACHINE_DEV2" "docs" push --docs
...
@test "M7: Merge preserves parent chain" {
    ...
    local commit
    commit=$(git ls-remote "$TEST_DIR/remote.git" "refs/refsync/sync/$TEST_USER/$MACHINE_DEV1/docs" 2>/dev/null | cut -f1)
