...
@test "R6: Amended change updates bookmark" {
    ...
    local sha_before
    sha_before=$(git ls-remote "$TEST_DIR/remote.git" "refs/refsync/sync/$TEST_USER/$MACHINE_LAPTOP/revs/$change_id" | cut -f1)
    [[ -n "$sha_before" ]]
    ...
    local sha_after
    sha_after=$(git ls-remote "$TEST_DIR/remote.git" "refs/refsync/sync/$TEST_USER/$MACHINE_LAPTOP/revs/$change_id" | cut -f1)
    [[ -n "$sha_after" ]]
...
@test "R8: --dry-run doesn't modify state" {
    ...
    # Push with --dry-run
    run run_refsync "$MACHINE_LAPTOP" push --dry-run
    [[ "$status" -eq 0 ]]
...
@test "R11: --both syncs revisions and docs" {
    ...
    # Push with --both
    REFSYNC_DOCS="docs" run_refsync "$MACHINE_LAPTOP" push --both
