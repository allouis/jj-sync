...
@test "E1: Non-colocated repo works" {
    ...
    # Push
    REF_SYNC_USER="$TEST_USER" REF_SYNC_MACHINE="noncoloc" REF_SYNC_REMOTE="sync" "$REF_SYNC" push
...
@test "E2: Missing remote errors gracefully" {
    ...
    run env REF_SYNC_USER="$TEST_USER" REF_SYNC_MACHINE="$MACHINE_LAPTOP" REF_SYNC_REMOTE="nonexistent" "$REF_SYNC" push
...
@test "E3: Not in a repo errors gracefully" {
    ...
    run env -u REF_SYNC_DOCS REF_SYNC_USER="$TEST_USER" REF_SYNC_MACHINE="laptop" REF_SYNC_REMOTE="sync" "$REF_SYNC" push
...
@test "E4: Unknown command errors gracefully" {
    ...
    run env REF_SYNC_USER="$TEST_USER" REF_SYNC_MACHINE="$MACHINE_LAPTOP" REF_SYNC_REMOTE="sync" "$REF_SYNC" unknown_command
...
@test "E7: Machine name sanitized" {
    ...
    REF_SYNC_USER="$TEST_USER" REF_SYNC_MACHINE="my@machine:with/special" REF_SYNC_REMOTE="sync" "$REF_SYNC" push
...
@test "E8: Empty push succeeds silently" {
    ...
    run run_ref_sync "$MACHINE_LAPTOP" push
...
@test "E9: Pull with no remote refs succeeds" {
    ...
    run run_ref_sync "$MACHINE_DEV1" pull
...
@test "E10: Status works with no sync state" {
    ...
    run run_ref_sync "$MACHINE_LAPTOP" status

    [[ "$status" -eq 0 ]]
    [[ "$output" =~ "ref-sync status" ]]
}

@test "E11: Clean with no sync state succeeds" {
    cd_to_machine "$MACHINE_LAPTOP"

    run run_ref_sync "$MACHINE_LAPTOP" clean --force
...
@test "E12: Deeply nested doc files work" {
    ...
    # Pull on dev-1
    run_ref_sync_with_docs "$MACHINE_DEV1" "docs" pull --docs
