...
@test "D4: Multiple doc dirs work" {
    ...
    run_refsync_with_docs "$MACHINE_LAPTOP" "ai/docs .claude" push --docs

    # Pull on dev-1
    cd_to_machine "$MACHINE_DEV1"
    run_refsync_with_docs "$MACHINE_DEV1" "ai/docs .claude" pull --docs
...
@test "D5: Empty doc dir - no error" {
    ...
    run run_refsync_with_docs "$MACHINE_LAPTOP" "ai/docs" push --docs
    [[ "$status" -eq 0 ]]
}
...
@test "D6: Missing doc dir - skip with warning" {
    ...
    run run_refsync_with_docs "$MACHINE_LAPTOP" "nonexistent/dir" push --docs
    [[ "$status" -eq 0 ]]
...
@test "D10: Doc commit has parent chain" {
    ...
    # Get first commit
    local first_commit
    first_commit=$(git ls-remote "$TEST_DIR/remote.git" "refs/refsync/sync/$TEST_USER/$MACHINE_LAPTOP/docs" | cut -f1)
    ...
    # Get second commit
    local second_commit
    second_commit=$(git ls-remote "$TEST_DIR/remote.git" "refs/refsync/sync/$TEST_USER/$MACHINE_LAPTOP/docs" | cut -f1)
...
@test "D12: Failed extraction leaves existing files untouched" {
    ...
    git -C "$TEST_DIR/remote.git" update-ref \
        "refs/refsync/sync/$TEST_USER/$MACHINE_LAPTOP/docs" "$commit_sha"
    ...
    run run_refsync_with_docs "$MACHINE_DEV1" "ai/docs" pull --docs
    [[ "$status" -ne 0 ]]
