...
# Path to ref-sync script
REF_SYNC="$PROJECT_ROOT/ref-sync"
...
assert_bookmark_exists_remote() {
    ...
    refs=$(git ls-remote "$TEST_DIR/remote.git" "refs/ref-sync/$bookmark" 2>/dev/null)
    if [[ -z "$refs" ]]; then
...
count_remote_bookmarks() {
    local pattern="$1"
    git ls-remote "$TEST_DIR/remote.git" "refs/ref-sync/$pattern" 2>/dev/null | wc -l | tr -d ' '
}
...
# Run ref-sync with test environment
# Usage: run_ref_sync <machine> [args...]
...
run_ref_sync() {
    ...
    REF_SYNC_USER="${REF_SYNC_USER:-$TEST_USER}" \
    REF_SYNC_MACHINE="${REF_SYNC_MACHINE:-$machine}" \
    REF_SYNC_REMOTE="${REF_SYNC_REMOTE:-sync}" \
    REF_SYNC_GC_REVS_DAYS="${REF_SYNC_GC_REVS_DAYS:-7}" \
    REF_SYNC_GC_DOCS_MAX_CHAIN="${REF_SYNC_GC_DOCS_MAX_CHAIN:-50}" \
        "$REF_SYNC" "$@"
}

# Run ref-sync with docs configured
# Usage: run_ref_sync_with_docs <machine> <docs_dirs> [args...]
run_ref_sync_with_docs() {
    local machine="$1"
    local docs_dirs="$2"
    shift 2
    ...
    REF_SYNC_USER="${REF_SYNC_USER:-$TEST_USER}" \
    REF_SYNC_MACHINE="$machine" \
    REF_SYNC_REMOTE="sync" \
    REF_SYNC_DOCS="$docs_dirs" \
        "$REF_SYNC" "$@"
}
